using namespace std;
void DAC_Scan(const int ich, int RN_scan1, int RN_scan2, int RN_scan3, int RN_scan4, int RN_scan5, int RN_scan6, int RN_scan7, int RN_scan8, int RN_scan9, int RN_scan10, int RN_scan11, int RN_scan12){
  const int Scan_num=12;
  int Color[12]={600,603,618,616,632,634,402,416,418,420,435,433};
  char name[200];
  int RN_scan[Scan_num]={RN_scan1,RN_scan2,RN_scan3,RN_scan4,RN_scan5,RN_scan6,RN_scan7,RN_scan8,RN_scan9,RN_scan10,RN_scan11,RN_scan12};
  ...
  for(int ifem=1; ifem<2; ++ifem){
    ...
    sprintf(filename,"dacScan_F%d_R%d_chip%d.png",ifem,RN_scan1,ich);
    c2-> Print(filename);
  }
  /***************************************************************************/
} 
